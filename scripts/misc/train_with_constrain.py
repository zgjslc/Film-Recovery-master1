    ...
    for batch_idx, data in enumerate(train_loader):
        ...
        if batch_idx % args.log_intervals == 0:
            print(
                ...
                'cons_t2d:{:.6f}||cons_n2d:{:.6f}||cons_d2n:{:.6f}'.format(epoch, batch_idx + 1,
        ...
        if isWriteImage:
            if batch_idx == (len(train_loader.dataset) // args.batch_size)-1:
                ...
                for k in range(5):
                    ...
                    write_image_01(mask_gt.transpose(0, 1).transpose(1, 2)[:, :, 0],
...
def test(args, model, device, test_loader, criterion, epoch, writer, output_dir, isWriteImage, sstep):
            ...
            if isWriteImage:
                if batch_idx == (len(test_loader.dataset) // args.test_batch_size)-1:
                    ...
                    for k in range(args.test_batch_size):
                        ...
                        write_image_tensor(cmap_pred, output_3d_pred, 'gauss', mean=[0.100, 0.326, 0.289],
                                           std=[0.096, 0.332, 0.298])
                        write_image_tensor(depth_pred, output_depth_pred, 'gauss', mean=[0.316], std=[0.309])
                        write_image_tensor(normal_pred, output_normal_pred, 'gauss', mean=[0.584, 0.294, 0.300],
                        ...
                        write_image_tensor(cmap_gt, output_cmap_gt, 'gauss', mean=[0.100, 0.326, 0.289],
                                           std=[0.096, 0.332, 0.298])
                        write_image_tensor(depth_gt, output_depth_gt, 'gauss', mean=[0.316], std=[0.309])
                        write_image_tensor(normal_gt, output_normal_gt, 'gauss', mean=[0.584, 0.294, 0.300],
                                           std=[0.483, 0.251, 0.256])
    ...
    if args.calculate_CC:
        ...
        cc_dewarp_ab = cc_dewarp_alb / (len(test_loader.dataset) / args.test_batch_size)
    if args.calculate_CC:
    ...
    if args.write_txt:
        ...
        f.write(
            'Epoch: {} \t Test Loss: {:.6f}, \t ab: {:.4f}, \t cmap: {:.4f}, \t uv: {:.6f}, \t normal: {:.4f}, \t depth: {:.4f}, \t back: {:.6f} , \t constrain 3d to normal: {:.4f}, \t constrain 3d to depth: {:.4f}, \t constrain normal to depth: {:.4f}, \t constrain depth to normal: {:.4f}, CC_uv: {}\t CC_cmap: {}\t CC_ab: {}\t CC_bw: {}\t CC_dewarp_ori: {}\t CC_dewarp_ab: {}\n'.format(
                ...
                test_cons_n2d.item(), test_cons_d2n.item(), cc_uv, cc_cmap, cc_ab, cc_bw, cc_dewarp_ori,
                cc_dewarp_ab))
        f.close()
    if args.write_summary:
        print('sstep', sstep)
